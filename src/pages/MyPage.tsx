...
// Items per page in the 좋아요한 추억 / 나의 댓글 lists; more shows pagination.
const LIKED_PER_PAGE = 5
...
export default function MyPage({ onOpenMemory, onLogout }: Props) {
  ...
  // Memories the current user has liked (public feed filtered by likedByMe).
  useEffect(() => {
  ...
  const totalLikedPages = Math.max(1, Math.ceil(liked.length / LIKED_PER_PAGE))
  const pageLiked = liked.slice((likedPage - 1) * LIKED_PER_PAGE, likedPage * LIKED_PER_PAGE)
          ...
          <div className="mypage-card">
            {pageLiked.map((m) => (
              <div key={m.memoryId} className="mypage-comment">
                ...
                <span className="mypage-like-badge" aria-label={`좋아요 ${m.likeCount ?? 0}`}>
